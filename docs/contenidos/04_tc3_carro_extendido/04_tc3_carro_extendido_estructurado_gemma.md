...
En esta implementación, añadiremos una nueva opción para forzar un **reinicio** del sistema, adicional al uso del botón de la visualización asociado a `SFCReset`. El sistema se reiniciará tras pulsar el **pulsador de marcha y el de parada de manera simultánea** durante un tiempo determinado (3s por defecto).
...
//  - DesactivarAlarma <-- tras desenclavar el pulsador de emergencia
...
La señal `OrdenReinicio` se le pasará al **FB** del `Director` como entrada:
...
el cual cambiará el modo del sistema al `A6` (**puesta en estado inicial**).

```pascal
// en FB_Director
IF OrdenEmergencia THEN
    Modo := E_GEMMA.D1_ParadaEmergencia;
ELSIF OrdenReinicio THEN
    Modo := E_GEMMA.A6_PuestaEstadoInicial; <-- evolución a A6
ELSE
```
...
Otra de las condiciones que van a provocar que el sistema vuelva a su estado inicial es la **detección de salida del estado de emergencia (`D1`)**. En nuestro sistema, cuando se detecte la pulsación del botón de emergencia, el `Director` pasará al modo `D1` (**para de emergencia**) y, cuando se desenclave la seta de emergencia, pasará al estado `A6` (**puesta en estado inicial**).
...
Para evitar mantener la parte operativa conectada de manera indefinida, se ha añadido al programa un temporizador de desconexión que monitoriza el tiempo que el sistema está **conectado y en reposo**. Tras un determinado tiempo en esta situación, el sistema se desconectará automáticamente.
...
Nótese como ahora tenemos el *placeholder* de `%s` (*string*) y cómo se especifica la lista de texto y el índice a utilizar para determinar la descripción a mostrar (el modo, en este caso).

## Mejoras y extensiones

Además de las funcionalidades anteriores, se han introducido una serie de mejoras a la implementación estructurada.

### Gestor de comandos

Se ha añadido un nuevo método `m_GestorComandos` para la gestión de los comandos de **Reinicio** y **Pausa**.

Para el **reinicio**, se ejecuta el código del temporizador de reinicio manual [(➡️)](#temporizador-de-reinicio-manual) y se define la orden de reinicio completa incluyendo los tres casos contemplados:
...
En la llamada al `Director`, se copia la variable de salida `Modo` del bloque funcional en la variable `ModoActual`, que es la que se utiliza en **todos** los métodos de la estación, además de en la visualización.
...
En esta versión vamos a inicializar el valor de `ManiobrasPendientes` cuando se produzca la transición de entrada al modo `F2` (**Secuencia de Preparación**). Para ello utilizaremos el valor de `ModoActual` y el de otra variable que llamaremos `ModoAnterior` que guarda el estado en el ciclo anterior:
...
La actualización de la variable `ModoAnterior` se realiza al final de cada ciclo:
...
Finalmente, tenga en cuenta que esta inicialización la realizaremos solo cuando estemos produciendo por lotes (lo que se controla con un botón en la visualización) y el número de maniobras pendientes valga `0` (es decir, se ha terminado el lote anterior, si lo hubiera habido).

![Producción por lotes](../../images/04_tc3_carro_extendido/Carro_Extendido_Estructurado_GEMMA_ProduccionLotes.png){width=300px}

### Gestión del panel

En el método `m_GestorPanel` se ha introducido el uso de `ModoActual` para condicionar la activación de los elementos `o_LamparaAlarma` y `o_LamparaParada` en el panel:
